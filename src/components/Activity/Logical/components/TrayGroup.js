    ...
    {trays &&
      trays.map((element, index, array) => (
        <Group type={element.type}>
          ...
          { element.srcType && <Circle x={getAllingX(element.width, index, array.length) + element.width/2} y={240} radius={25} fill="white" stroke="black" />}
          { element.srcType && <KonvaImage x={getAllingX(element.width, index, array.length) + element.width/2 -17} y={220} width={30} height={40} image={imageFactory(element.srcType)} />}        
