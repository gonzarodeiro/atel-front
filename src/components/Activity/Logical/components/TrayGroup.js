      ...
      {trays &&
        trays.map((element, index, array) => {
          if (element.type !== 'RESULT') {
            return (
              <Group type={element.type} name={'group-' + element.type + '-' + index} height={180}>
                ...
                <Text id={'expectedQuantity' + element.id} text={element.expectedQuantity} x={getAllingX(element.width, index, array.length, width) + element.width / 2 - 10} y={120} height={20} width={20} fontVariant='bold' fontSize={35} align='center' verticalAlign='middle' strokeWidth={1} fill='black' shadowColor='white' shadowBlur={10} />
              </Group>
            );
          } else {
            return (
              <Group type={element.type} name={'group-' + element.type + '-' + index}>
                <KonvaImage id={'tray-' + index} name={element.id} key={element.id} x={getAllingX(element.width, index, array.length, width)} y={130} width={element.width} height={element.height} image={imageFactory(element.src)} />
